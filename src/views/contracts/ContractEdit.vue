<script setup>
    ...
    const {contract, getContract, updateContract, errors} = useContracts();
      ...
      <div>
        ...
        <div class="bg-white rounded shadow-lg p-4 px-4 md:p-8 mb-6">
          <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3">
            <form @submit.prevent="updateContract($route.params.id)">
            <div class="lg:col-span-2">
              <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
                <div class="md:col-span-5">
                  <label for="name">Name</label>
                    <input type="text" v-model="contract.name" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50">
                </div>
  ...
  <form @submit.prevent="updateContract($route.params.id)">
     <div class="w-full mb-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
         ...
         <div class="px-4 py-2 bg-white rounded-b-lg dark:bg-gray-800">
             ...
             <textarea v-model="contract.body" id="editor" rows="8" class="block w-full px-0 text-sm text-gray-800 bg-white border-0 dark:bg-gray-800 focus:ring-0 dark:text-white dark:placeholder-gray-400" placeholder="Write an article..." required></textarea>
         </div>
     </div>
